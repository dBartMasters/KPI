...
Author: Bart Masters
Date: 2024-02-23
DB: Snowflake PROD_ANALYTICS
Role: PROD_ANALYTICS_ROLE
Purpose: Calculate basic KPIs around users for basic KPIs like retention, CAC, LTV, and others

Reference: https://docs.google.com/spreadsheets/d/1L31pkFsoovDDQQ618GdtnImnGVYBrPmr1oN2gADAmsU/edit#gid=507258549 
*/

/*----------------------------------------------------*/
-- V2
/*----------------------------------------------------*/
/*----Query from Temma that gets revenue for what's in marketplace---------*/
-- explain
create or replace table PROD_ANALYTICS.PRODUCT_ANALYTICS.KPI_MP_FEE AS
with fees as (
    select feeable_id
    , feeable_type
    , sum(case
...
, SHIPMENT_ID
, SELF_NOTE_BODY
, BUYER_DEDUCTION::NUMBER AS BUYER_DEDUCTION
, WIRE_RECEIVER
, PAYABLE_DAYS1
, ADVANCED_PERCENTAGE
...
;
SELECT * FROM PROD_ANALYTICS.PRODUCT_ANALYTICS.KPI_MP_FEE LIMIT 100;
/*----KPI BASE---------------------------------------------*/
-- EXPLAIN
CREATE OR REPLACE TABLE PROD_ANALYTICS.PRODUCT_ANALYTICS.KPI_BASE AS
WITH LT AS (
    SELECT 
    DISTINCT 
    I.OPPORTUNITY_ID,
    I.FINANCE_DEAL_NUMBER,
    LT.SELLER_ID,
    LT.ALL_IN_REVENUE,
    LT.DEAL_NUMBER
    FROM 
    PROD_ANALYTICS.PRODUCT_ANALYTICS.VW_FINANCE_PS_DEAL_IDS I 
    JOIN
    PROD_ANALYTICS.PRODUCT_ANALYTICS.FINANCE_PS_LOANTAPE LT
    ON
    LT.DEAL_NUMBER = I.FINANCE_DEAL_NUMBER
)
SELECT O.NAME,
        O.ACCOUNT_CLASS__C,
        O.CLASS__C,
        ...
        --REVENUE
        O.EXPECTEDREVENUE,
        O.EST_MONTHLY_PS_REVENUE__C,
        O.WEIGHTED_PS_REVENUE__C,
        ...
        --Add PS revenue and other revenue
        ----If PS revenue from the analytics PSAR view is present, use it, otherwise, use the salesforce weighted PS revenue
        -- NVL(IFF(PS_REVENUE>0, PS_REVENUE, LT.U_SUM_REVENUE),0) + NVL(F_REVENUE,0) AS REVENUE,
        ----If loan tape revenue is there, use it, otherwise use the PSAR revenue
        LT.ALL_IN_REVENUE,
        NVL(IFF(LT.ALL_IN_REVENUE>0, LT.ALL_IN_REVENUE, PS_REVENUE),0) + NVL(F_REVENUE,0) AS REVENUE,
        --
        O.ISCLOSED,
        ...
        ROW_NUMBER() OVER (
            PARTITION BY NVL(LT.SELLER_ID, A.ID)
        ...
        MAX(CASE WHEN PRODUCT_LABEL <> 'PreSeason' then 1 else 0 end) OVER (
            PARTITION BY NVL(LT.SELLER_ID, A.ID)
        ) AS OTHER_PRODUCT_INDICATOR,        
        CASE
        WHEN FIRST_PRODUCT IN ('InSeason','QP96', 'QP80','QP Marine','InSeason w/Quote','QP50','1-Click','RP Trade','Factoring','End Buyer','PCP') THEN 'QP'
        ...
        CASE
        WHEN LAST_PRODUCT IN ('InSeason','QP96', 'QP80','QP Marine','InSeason w/Quote','QP50','1-Click','RP Trade','Factoring','End Buyer','PCP') THEN 'QP'
        ...
        CASE
        WHEN PRODUCT_LABEL IN ('InSeason','QP96', 'QP80','QP Marine','InSeason w/Quote','QP50','1-Click','RP Trade','Factoring','End Buyer','PCP') THEN 'QP'
    ...
    FROM PROD_ANALYTICS.SALESFORCE.OPPORTUNITY O
        ...
        LEFT JOIN LT ON O.ID = LT.OPPORTUNITY_ID
...
count(*) as lines,
sum(1) as lines,
sum(case when ACTIVITY_DATE is NULL then 0 else 1 end) as nonnull
FROM PRODUCT_ANALYTICS.KPI_BASE 
group by 1
LIMIT 100;
/*----KPI DIM---------------------------------------------*/
CREATE OR REPLACE TABLE PRODUCT_ANALYTICS.KPI_DIM AS
SELECT
DISTINCT
...
WHERE ACCOUNT_CLASS='Mega Whale' and FIRST_PRODUCT_CATEGORY='QP'
ORDER BY 3
LIMIT 100;
/*------NEW CHURN QUERY V2---------------------------------------------*/
EXPLAIN
...
/*------V2 TOTAL ACTIVE CUSTOMER ACCOUNTS---------------------------------------------*/
--closeyr-product-v2
---- HAVE TO GROUP THIS WAY: if we sum over months, we double count, if we max, we undercount because account can be in beginning of window but not end, and others vice versa
SELECT * FROM PROD_ANALYTICS.PRODUCT_ANALYTICS.KPI_BASE LIMIT 100;
WITH MG AS (
    SELECT ID AS ACCOUNT_ID,
        CLOSED_MONTH,
        FIRST_PRODUCT_CATEGORY,
...
RETAINED AS(
    SELECT KD.YEAR_IDX,
        ...
        SUM(R.REVENUE) AS REVENUE
    FROM PRODUCT_ANALYTICS.KPI_DIM KD
        LEFT JOIN MG AS R ON KD.MONTH_IDX BETWEEN R.CLOSED_MONTH AND R.RET_DATE_MAX AND KD.FIRST_PRODUCT_CATEGORY = R.FIRST_PRODUCT_CATEGORY
        -- where account_id ='0015w00002KjNA0AAN'
    GROUP BY KD.YEAR_IDX,
        KD.FIRST_PRODUCT_CATEGORY
)
SELECT * FROM RETAINED LIMIT 100;
/*------V2 Active customer accounts by products used---------------------------------------------*/
--CLOSEYR-NUMPRODUCTS
---- HAVE TO GROUP THIS WAY: if we sum over months, we double count, if we max, we undercount because account can be in beginning of window but not end, and others vice versa
SELECT ID AS ACCOUNT_ID,
        YEAR(ACTIVITY_DATE) AS YR_CLOSE,
        COUNT(DISTINCT PRODUCT_LABEL) AS PRODUCT_CATS
    FROM PROD_ANALYTICS.PRODUCT_ANALYTICS.KPI_BASE 
    GROUP BY ACCOUNT_ID,
        YR_CLOSE
        ;